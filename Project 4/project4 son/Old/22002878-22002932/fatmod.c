...
void list_files(int fd) {
    ...
    while (dir_entry->name[0] != 0x00) {
        //size > 0 statement deleted from the following line
        if (dir_entry->name[0] != 0xe5 && (dir_entry->attr & ATTR_DIRECTORY) == 0) {
            char filename[13];
...
void create_file(int fd, char *filename) {
    ...
    //dir_entry_ptr->size = 0; This line is deleted

    //These lines added
    for (int i = strlen(filename); i < 8; i++) {
        dir_entry_ptr->name[i] = ' ';
    }
    dir_entry_ptr->attr = 0x20;
    unsigned int new_cluster = 2;
    unsigned char fat[SECTORSIZE];
    int fat_sector = boot_sector.reserved;
    while (1) {
        if (readsector(fd, fat, fat_sector) == -1) {
            fprintf(stderr, "Failed to read FAT sector\n");
            return;
        }
        for (unsigned int i = 0; i < SECTORSIZE / 4; i++) {
            unsigned int fat_entry = ((unsigned int *)fat)[i];
            if ((fat_entry & 0x0FFFFFFF) == 0) {
                ((unsigned int *)fat)[i] = 0x0FFFFFFF; // Mark as end of cluster chain
                new_cluster = fat_sector * (SECTORSIZE / 4) + i;
                if (writesector(fd, fat, fat_sector) == -1) {
        ...
        if (new_cluster != 2) break;
        fat_sector++;
    }
    dir_entry_ptr->start = new_cluster; // 0 changed to new_cluster
